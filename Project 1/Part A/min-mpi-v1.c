...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    ...
    /* Find local min */
    global_min = find_min(search_array, sub_len);
    printf("Min for P%d is %d.\n", rank, global_min);

    /* Get back the minima from the others */
    for (proc = 1; proc < num_proc; proc++) {
      MPI_Recv(&local_min, 1, MPI_INT, proc, 0, MPI_COMM_WORLD, &status);
      if (local_min < global_min) {
        global_min = local_min;
      }
    }

    /* Display the global min */
    printf("Global min: %d\n", global_min);
    timeEnd = MPI_Wtime();
